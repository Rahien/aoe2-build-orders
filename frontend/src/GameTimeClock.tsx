...
const GameTimeClock: React.FC<IGameTimeClockProps> = ({
  gameTime,
  setGameTime,
}) => {
  ...
  useEffect(() => {
    ...
    gameTimeMinutesTemp =
      gameTimeMinutesTemp.length > 1
        ? gameTimeMinutesTemp
        : `0${gameTimeMinutesTemp}`;
    let gameTimeSecondsTemp = "" + (Math.floor(gameTime) % 60);
    gameTimeSecondsTemp =
      gameTimeSecondsTemp.length > 1
        ? gameTimeSecondsTemp
        : `0${gameTimeSecondsTemp}`;
    setGameTimeMinutes(gameTimeMinutesTemp);
    setGameTimeSeconds(gameTimeSecondsTemp);
  }, [gameTime]);

  const updateGameTimeMinutes = (
  ...
  return (
    <div className="time-display">
      <input
        type="number"
        onKeyDown={updateGameTimeMinutes}
        step={1}
        min={0}
      ...
      <input
        type="number"
        onKeyDown={updateGameTimeSeconds}
        step={1}
        min={0}
        max={60}
